...
public class RezSessionController {
    ...
    public void start() {
        while(inGame) {
            if(logic.getBoard().isAllSet()) {
                System.exit(0);
            }
            if(logic.isInitMode()) {
                Location loc = getCurrentPlayer().getItem();
                ...
                Move mv = getCurrentPlayer().moveItem();
                if(logic.canMove(mv)) {
                    logic.move(mv);
                    doRezCheck(mv.getTo());
                    logic.changePlayer();
